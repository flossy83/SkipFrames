...
const AVS_Linkage* AVS_linkage = nullptr;  // linkage to avisynth.dll
...
public:
    ...
    skipahead(0), prevframe(0), seekframes(-1), seqframes(0), prevseqframe(0), prevframeOnSeek(-1), currframeOnSeek(-1),
    audioRelativeFrame(-1), audioRelativeFrameFound(-1), audioSkipaheadFrames(0),
    ...
    void __stdcall SetFrameProps(PVideoFrame pvidframe, int currframe, int outframe, bool isProbe, IScriptEnvironment* env){
        ...
        env->propSetInt(rwProps, "_SkipFrames_06_Bool_IsProbe", isProbe ? 1 : 0, 0);   // local
		env->propSetInt(rwProps, "_SkipFrames_07_Int_InFrame", currframe, 0);          // local
		env->propSetInt(rwProps, "_SkipFrames_08_Int_SkipAhead", skipahead, 0);
		env->propSetInt(rwProps, "_SkipFrames_09_Int_OutFrame", outframe, 0);          // local
    ...
    PVideoFrame __stdcall GetFrame(int n, IScriptEnvironment* env) override {
		...
		if (n <= ignorerange-1){
			...
			if (debug == 1){ SetFrameProps(out, n, outframe, true, env); }                
            return out;
		}
